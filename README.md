...
**Default: Claude Sonnet 5 on the Anthropic API** (`MODEL_PROVIDER=anthropic`, `MODEL_NAME=claude-sonnet-5`, key in `.env.dev`). Thinking is disabled per call. Auxiliary calls (self-state summaries, session distillation) go to Haiku 4.5 through Claude Code (`AUX_MODEL_PROVIDER=claude_code`, bare mode).

- **No temperature on Claude 5-series models.** Sonnet 5, Opus 5.5 and Fable 5.1 reject `temperature` ("deprecated for this model"). For them the SAMPLING_* curve is skipped and a one-line per-phase prose hint (`prompts.PHASE_HINTS`) carries the phase instead. Haiku 4.5 and Sonnet 4.5 still take temperature, clamped to 1.0 with `top_p` dropped. The `temperature` column in `tokens` records the nominal curve either way.
- **Soft step budget.** An instruct model handed a buffer that ends mid-sentence restarts that sentence, which reads as a stutter. So `max_tokens_per_step` is soft: the request allows up to 1.5×, and the stream stops at the first sentence end past the budget (or the last word boundary at the cap). Base models keep a hard server-side limit.

**Why not Claude Code for the dreamer.** `MODEL_PROVIDER=claude_code` still works: each step runs `claude -p` with no tools, no thinking, and the dream system prompt. But the CLI attaches context the dream reads as residue. A logged-in account sends the account email, working directory, git status, model identity and date. `CLAUDE_CODE_BARE=true` strips all of that except `Today's date is …`, and even that one line took over session 22: the dream kept writing the date and "the reminder" into the text. Under Claude Code, `sampler.detect_date_leak` cuts ISO dates, and (logged-in only) `sampler.detect_harness_leak` cuts email, path, git and model-name mentions. Recovery also rechecks the kept text against every detector.

**Other providers** go through litellm: `MODEL_PROVIDER=ollama` / `openai` with full temperature control. For local base models set `MODEL_PROVIDER=ollama`, `MODEL_MODE=base`, and point `AUX_MODEL_*` at an instruct model, since a base model can't follow the summarize/distill instructions.
